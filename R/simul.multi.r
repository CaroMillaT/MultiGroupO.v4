...
fun.sim<-function(g,mean1,d,n.var,sds2,corr)
{
  ...
  for(i in 1:(length(g)+1))
  {
    if(i<=length(g)){
      ...
      if(i==1){
        mu[[i]]<- seq(from =means[i],to=means[i]+1,length=n.var[1])
        x1[[i]] <- rmvnorm(g[i], mean =mu[[i]],sigma=liscovar[[i]])
      }
      else if(i==2){
        mu[[i]]<- seq(from =means[i],to=means[i]+0.5,length=n.var[1])
        x1[[i]] <- rmvnorm(g[i], mean =mu[[i]],sigma=liscovar[[i]])
      }
      else if(i==3){
        mu[[i]]<- seq(from =means[i],to=means[i]+1,length=n.var[1])
        ...
        mu[[i]]<- seq(from =means[i],to=means[i]+1.5,length=n.var[1])
        x1[[i]] <- rmvnorm(g[i], mean =mu[[i]],sigma=liscovar[[i]])
      }
      else if(i==5){
